...
// getConfigCommand returns a ztp configuration command
func getConfigCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "config",
		Short: "Read and update the ztp configuration",
	}
...
// getConfigGetCommand returns a ztp configuration get command
...
// getConfigSetCommand returns a ztp configuration set command
...
func initConfig() error {
	...
	if err := viper.ReadInConfig(); err != nil {
		if _, ok := err.(*viper.ConfigFileNotFoundError); !ok {
			...
			f, err := os.Create(home + "/.onos/ztp.yaml")
			if err != nil {
				return err
			}
			defer f.Close()
